...
class CvFinanceAdvisor:
	...
	def drawContents(self):
		...
		yLocation += self.Y_SPACING
		screen.setLabel(self.getNextWidgetName(), "Background", u"<font=3>" + localText.getText("TXT_KEY_FINANCIAL_ADVISOR_INTEREST", ()) + u" at " + unicode(gc.getDefineINT("INTEREST_RATE")/1000) + u"." + unicode(gc.getDefineINT("INTEREST_RATE")%1000) + u"%:</font>", CvUtil.FONT_LEFT_JUSTIFY, self.X_INCOME + self.TEXT_MARGIN, yLocation + self.TEXT_MARGIN, self.Z_CONTROLS + self.DZ, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )
		interestAmount = gold * gc.getDefineINT("INTEREST_RATE") / 100000
		screen.setLabel(self.getNextWidgetName(), "Background", u"<font=3>" + unicode(interestAmount) + "</font>", CvUtil.FONT_RIGHT_JUSTIFY, self.X_INCOME + self.PANE_WIDTH - self.TEXT_MARGIN, yLocation + self.TEXT_MARGIN, self.Z_CONTROLS + self.DZ, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )

		iIncome += interestAmount
